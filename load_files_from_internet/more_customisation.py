...
def graph_data(stock):
	...
	## changing the color of labels of ax1 subplot
	ax1.xaxis.label.set_color('c')
	ax1.yaxis.label.set_color('g')
	## setting the y axis scale or ticks
	ax1.set_yticks([100,200,300,400])
	## fill the color between the bottom and the price number specified
	## plt.fill_between(x_entity, y_entity, y_entity number, opaqueness)
	## alpha is the degree of opaquness
	## ax1.fill_between(date, openp, 250, alpha=0.3)
	## also we can have logic to fill between the graph
	ax1.fill_between(date, openp,openp[0], where=(openp > openp[0]), facecolor='g')
	ax1.fill_between(date, openp, openp[0], where=(openp < openp[0]), facecolor='r')
	plt.xlabel('dates')
	plt.ylabel('price')
